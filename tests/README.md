...
| `test_yolo_detector.cpp` | 5 | YoloDetector / YoloRoiProvider 端到端 | `best.onnx` + `data/fixtures/` 图片 (缺失 SKIP) | 状态码 / 不崩溃 |
...
### 4.9 `test_yolo_detector.cpp` — YOLO 检测器端到端冒烟 (5 用例)

**被测模块**: `YoloDetector` (模型加载 / detect 状态码) + `YoloRoiProvider` (YOLO → RoiGroup 外观)
**输入数据**: `best.onnx` 模型（NMS-export 已解码格式 `[1,300,6]`）+ `tests/data/fixtures/synthetic_akaze/left_000.png` 图片（仅冒烟，不强断言检测框）。模型或图片缺失时用例 SKIP（不 FAIL）。
...
> ⚠️ **冒烟定位**: fixture 合成图非真实目标，检测结果无确定性保证，故只断言状态码/不崩溃。（原 `test_yolo_decode` 解码纯逻辑单测已随 2026-08-31 切回 best.onnx 移除——NMS-export 格式无需客户端解码/NMS。）

---

### 4.10 `test_synthetic_pipeline.cpp` — 合成图全流程 (8 用例)
...
./tests/test_yolo_detector   # 需 best.onnx 在位, 否则 SKIP
...
| ONNX Runtime | 可选, `-DGPNP_ONNXRUNTIME_ROOT=<path>` | `test_input_system` / `test_yolo_detector` 跳过编译 |
| YOLO 模型 | 可选, 运行目录需含 `best.onnx` | `test_yolo_detector` 相关用例 SKIP |
...
| 2026-08-31 | `test_yolo_decode.cpp` | 移除 — YOLO 模型切回 `best.onnx` (NMS-export `[1,300,6]` 已解码), 客户端无需解码/NMS | 删除 `YoloDecode.hpp` 纯函数与 10 用例单测; `postprocess()` 恢复 `06e1aa7^` 版内联解析 |
| 2026-08-31 | `test_yolo_detector.cpp` / `test_synthetic_pipeline.cpp` | 模型路径 `yolo_onnx/yolov8n.onnx` → `best.onnx` | 用例逻辑不变; `tests/CMakeLists.txt` 同步移除 `test_yolo_decode` 引用 |
