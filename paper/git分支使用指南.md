...
### ⚠️ 核心痛点：签出（Checkout/Switch）分支时，本地未提交的修改如何处理？

当你尝试在本地代码未提交（有暂存区 `add` 或工作区修改）的情况下切换分支时，会发生以下两种情况：

#### 情况 A：没有冲突（修改的文件在目标分支中没有变动）

* **结果** ：Git 允许你成功切换分支。
* **表现** ：你本地的修改（包括暂存和未暂存的代码）会**直接被带到新分支**中。
* **避坑提示** ：这在多人协同开发中容易引发混乱，极易导致你不小心把 A 分支的修改混进 B 分支并提交上去。

#### 情况 B：发生冲突（你修改的文件在目标分支中也有更新）

* **结果** ：Git 会 **强行拦截并拒绝切换** ，抛出经典报错：
...
* **原因** ：Git 具有保护机制，防止你本地尚未保存的劳动成果被新分支的代码强行覆盖抹去。

---

### 💡 进阶实战：跨分支 Stash 传递代码的底层机制

在实际开发中，先在 A 分支 `git stash`，切到 B 分支后再 `git stash pop`，是高频使用的**“代码跨分支搬运”**技巧。

```
[A 分支] ──(1. 写了代码)──► (2. git stash 存入公共栈)
                                 │
                                 ▼ (3. 切换分支)
[B 分支] ◄──(5. 成功应用或解决冲突) ◄── (4. git stash pop 取出栈顶)
```
...
如果 A 分支修改的文件，在 B 分支上没有冲突：

null. **代码完全移入 B 分支**的工作区。
null.  **暂存区状态重置** ：原本在 A 分支运行过 `git add` 的文件，在 B 分支恢复后默认会 **退回工作区** （变成未暂存状态）。
      * *💡 进阶技巧：若想在 B 分支上恢复时继续保持原有的“已暂存”和“未暂存”区别，请执行 *`git stash pop --index`。
null.  **暂存记录自动销毁** ：该 `stash` 记录会自动从暂存栈中移除（Drop）。

##### 🔴 效果二：存在代码冲突（触发 Git 双重保护机制）

如果 B 分支在相同文件的相同行有新提交，导致无法直接应用：

null.  **报冲突错误** ：工作区中冲突文件的冲突位置会留下经典的 `<<<<<<<` 和 `>>>>>>>` 冲突标记，此时需要你手动解决冲突。
null.  **🚨 终极保护机制（Stash 记录不会被删除）** ：**因为发生了冲突，Git 绝对不会自动销毁这个 stash 记录！**
      * 即使你在 B 分支手动解决冲突时把代码改乱了，你随时可以用 `git checkout .` 撤销，并重新运行 `git stash apply` 再次尝试，这极大地保障了代码的安全。
null.  **手动清理** ：当你解决完冲突并提交代码后，必须手动运行下述命令清空暂存：
      ``      git stash drop  # 手动删除栈顶的那个 stash 记录      ``

---

#### 🛠️ 商业项目中的 4 种标准解决方案：

##### 1. 方案一：推荐使用 Stash 暂存挂起（最安全、最常用）

如果你当前的代码只写了一半，还不具备提交（Commit）的意义：

```
# 1. 连同未跟踪的新文件一起，将修改安全存入暂存栈中（工作区瞬间变干净）
git stash -u
...
##### 2. 方案二：先 Commit 提交（如果阶段性工作已完成）

如果你当前写的代码已经是一个相对完整的节点，可以直接在当前分支提交。

```
git add .
git commit -m "feat(auth): temporary commit for switching branches"

# 切换去干别的事...
# 办完事切回来后，如果想撤销这个临时提交、重新组织代码，只需执行：
git reset --soft HEAD~1
```

##### 3. 方案三：带过去并自动合并（不推荐，易混淆）

如果你明确知道本地修改是需要带到另一个分支去的，可以使用 Git 的合并签出：

```
# 强制携带本地修改切换分支，并尝试在目标分支上进行自动 merge
git switch -m target-branch

# 注意：如果存在冲突，你必须在切换过去后的目标分支上，当场手动解决冲突。
```

##### 4. 方案四：彻底放弃本地修改（慎用，代码会消失）

如果你确认本地写的所有修改都是无用代码，想回到该分支上一次提交的干净状态：

```
# 放弃工作区未暂存的所有修改
git restore .

# 如果已经 git add 了，想要彻底清空回到上一次 Commit：
git reset --hard HEAD
```
...
It能保证开发分支上再多的中间记录，也绝对不会污染公共 `develop` 分支的历史。
